...
```{r}
# writing code that looks at a plot of deaths and cases simultaneously for a specific race in a specific state
covid_data_count |> 
  filter(state == "MA") |> 
  filter(race == "White") |>
  ggplot(aes(x = date)) + geom_line(aes(y=Cases), color= "black") + geom_line(aes(y=Deaths), color= "red") + labs(y="count", title = "Cases and Deaths for the White population in MA") 

```
```
2. Are there any conclusions you can draw?
It's quite challening to draw conclusions from a singular graph. But we can see how in the time frame from Oct - Feburary, the rate of growth for cases grew at a much quicker rate than the rate of deaths. 
3. What were you not able to do due to not having the R knowledge?
I wanted to plot barcharts where each bar would represent a race and the y-axis would be a proportion of the population that has gotten COVID. 
